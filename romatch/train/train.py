...
def train_step(
    ...
    **kwargs
    ...
    if hasattr(grad_scaler, "_scale"):
        wandb.log({"grad_scale": grad_scaler._scale.item()}, step=romatch.GLOBAL_STEP)
        if grad_scaler._scale < 1.:
            grad_scaler._scale = torch.tensor(1., device=grad_scaler._scale.device)

    romatch.GLOBAL_STEP = romatch.GLOBAL_STEP + romatch.STEP_SIZE

    wandb.log({"train/loss": l.item()}, step=romatch.GLOBAL_STEP)
